	...
	const char* UnAv) // M//ANDREA
{
	...
	Vertex* v1;
	Vertex* v2;
	for (itrN = m_hGraph.m_hNodeList.begin(); itrN != m_hGraph.m_hNodeList.end(); itrN++) {
		cout << "Node:" << (*itrN)->m_nNodeId << endl;
		v1 = m_hGraph.lookUpVertex((*itrN)->m_nNodeId, Vertex::VT_Channel_In, -1);
		v2 = m_hGraph.lookUpVertex((*itrN)->m_nNodeId, Vertex::VT_Channel_Out, -1);

		cout << "v1: " << v1->m_pOXCNode->getId() << endl;
		cout << "v2: " << v2->m_pOXCNode->getId() << endl;
	}
	cin.get();
...
inline bool NetMan::BBU_ProvisionNew(Connection *pCon)
{
	...
	if (bSuccess)
	{
		...
		assert(groomingConnections.size()==0);
...
inline bool NetMan::BBU_ProvisionHelper_Unprotected(Connection *pCon, Circuit& hPCircuit, Vertex *pSrc, Vertex *pDst)
{
	...
	// It means it's a connection changing bbu
	...
	if (this->connectionsChangingBBU.size() > 0) {
		...
		Connection* pFirstConChangingBBU;

		// Save the new path for the first connections in the connectionsChangingBBU list 
		pFirstConChangingBBU = (Connection*)*(this->connectionsChangingBBU.begin());
		pFirstConChangingBBU->m_pPathToUse = path;
		pFirstConChangingBBU->m_hCostToUse = precomputedCost;

		// each connection which is routed to the new bbu updates the path for the next connection to the new bbu
		if (this->connectionsChangingBBU.size() > 1) {
			...
			if (!(pCon->m_pPathToUse).empty()) {
				...
				for (itr = this->connectionsChangingBBU.begin(); itr != this->connectionsChangingBBU.end(); itr++)
				{
					...
					if (pCon == pConChangingBBU) {
						++itr;
						pConChangingBBU = (Connection*)(*itr);

						if (pConChangingBBU->m_nSrc == pCon->m_nSrc && pConChangingBBU->m_nDst == pCon->m_nDst) {
#ifdef DEBUG			
							cout << "Connection updating path is from " << pConChangingBBU->m_nSrc << " to "
								<< pConChangingBBU->m_nDst << " with seq number " << pConChangingBBU->m_nSequenceNo << endl;
#endif
				...
				connectionsChangingBBU.remove(pCon);
			}
		}
		else {
	...
	while (itr != hLinkList.end())
	{
		...
		switch (pLink->m_eSimplexLinkType)
		{
			...
			case SimplexLink::LT_UniFiber:
			{
				...
				if (pLink->m_pUniFiber->getSrc()->getId() == 46) {
					cout << "perchè usa questo link??" << endl;
					cin.get();
					cin.get();
				}
			...
			case SimplexLink::LT_Lightpath:
			{
				...
				if (pLink->m_pLightpath->getSrc()->getId() == 46) {
					cout << "perchè usa questo link??" << endl;
					cin.get();
				}
...
UINT NetMan::placeBBUHigh(UINT src, vector<OXCNode*>&BBUsList)
{
	...
	for (int j = 0; j < BBUsList.size(); j++)
	{
		...
		m_hGraph.Yen(p, pSrc, pDst, 2, AbstractGraph::LinkCostFunction::LCF_ByOriginalLinkCost);

		cout << "List of paths found: " << p.size() << endl;

		list<AbsPath*>::const_iterator itYen;
		itYen = p.begin();

		do {

			list<AbstractLink*> hCurrBPath = (*itYen)->m_hLinkList;

			list<AbstractLink*>::const_iterator itrPath = hCurrBPath.begin();
			SimplexLink* linkfrompath;
			...
			cout << "\tPath:" << endl;
			while (itrPath != hCurrBPath.end()) {

				linkfrompath = (SimplexLink*)(*itrPath);

				cout << "\tLink type: " << linkfrompath->getSimplexLinkType();

				if (linkfrompath->getSimplexLinkType() == SimplexLink::LT_Channel)
				{
					Vertex*pVertex = (Vertex*)linkfrompath->getSrc();
					//cout << pVertex->m_pOXCNode->getId() << "->";
					pVertex = (Vertex*)linkfrompath->getDst();
#ifdef DEBUGC
					cout << pVertex->m_pOXCNode->getId();
					cout << " (fiber " << linkfrompath->m_pUniFiber->getSrc()->getId()
						<< "->" << linkfrompath->m_pUniFiber->getDst()->getId() << ")" << endl;
#endif
				}
		...
		if (pathCost < UNREACHABLE)
		{
			...
			if (lat <= LATENCYBUDGET)  //-B: -> should be unnecessary using DijkstraLatency -> IT IS ABSOLUTELY NECESSARY! (read at the end of DijkstraHelperLatency)
			{
				...
				if ((pOXCsrc->m_nBBUNodeIdsAssigned > 0) && pOXCdst->getId() != pOXCsrc->m_nBBUNodeIdsAssigned && (pOXCdst->m_dCostMetric < bestCost)) {
					...
					bool capacityIsEnough = verifyCapacity(pOXCsrc, pOXCdst->getId());
					if (capacityIsEnough) {
				...
				if (src != lightpathSrc || (connType != 1 && connType != 2)) {
					if (!pLink->m_pLightpath->m_groomingAlreadydone) {
						...
						for (itr = pLink->m_pLightpath->m_hConnectionsUsingLightpath.begin(); itr != pLink->m_pLightpath->m_hConnectionsUsingLightpath.end(); itr++) {
							...
							if (src == lightpathSrc && (connType != 1 && connType != 2) && (pCon->m_nSrc != pCon->m_nDst)) {
					...
					else if (pLink->m_pLightpath->m_groomingAlreadydone) {
#ifdef DEBUG
						cout << "\t++ Cost of the link = " << pLink->getCost() << endl;
...
bool NetMan::verifyCapacity(OXCNode* pOXCSrc, UINT bestBBU) {
	...
	hPrimaryCost = m_hGraph.DijkstraLatency(pathToCheck, pSrc, pDst, AbstractGraph::LCF_ByOriginalLinkCost);
