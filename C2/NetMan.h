...
public:
	...
	void appendLightpath(Lightpath *, BandwidthGranularity eBW);
...
protected:
	...
	void PAC_DPP_NewCircuit(Circuit&, const list<AbstractLink*>&);
	...
	void SPAC_Opt_UpdateLinkCost_Primary_Lightpath(SimplexLink&, 
			const list<AbstractLink*>&, BandwidthGranularity);
	...
	void SPAC_SPP_UpdateLinkCost_Backup(Graph&, Circuit&,
			BandwidthGranularity, bool bIgnoreTx=false);
	...
	void updateGraphValidityAndCosts(OXCNode * pOXCSrc, BandwidthGranularity bwd);
	list<AbstractLink*> BBU_NewCircuit(Circuit & hCircuit, const list<AbstractLink*>& hLinkList, UINT, Connection*pCon);
	...
	void genAuxCUsList(int srcId, vector<OXCNode*>& auxBBUsList); //-L
	void genAuxCentralList(vector<OXCNode*>& auxBBUsList); //-L
	...
	void reinsertBBU(OXCNode* nodeToInsert);
	bool checkAvailabilityHotelNode(OXCNode * pOXCsrc);
	bool checkResourcesPoolNode(OXCNode * pOXCsrc, BandwidthGranularity backBwd);
	...
	UINT placeCUHigh(UINT src, vector<OXCNode*>& BBUsList);
	...
	list<AbstractLink*> removeGroomingLink(list<AbstractLink*>);
	Connection* checkLinks(Connection* pConFromDB, list<AbstractLink*>& savedPath, list<AbstractLink*>& path);
	bool checkifConnectionpresent(UINT seqNumber);
	void printPath(list<AbstractLink*>);
	void checkPath(UINT, UINT, LINK_COST);
public:
...
//////////////////////////// -B: ADDED BY ME
	...
	// -L: new function for the CU placement
	UINT findBestCUHotel(UINT src, BandwidthGranularity&, SimulationTime hTime);
	...
	void invalidateSimplexLinkDueToCapOrStatus(UINT bwd, UINT connType);

	void invalidateSimplexLinkDueToFreeStatus(BandwidthGranularity); //-B
	...
	float computeLatency(list<AbstractLink*>& path, Vertex* pSrc);

	float computeLatencyP3(list<AbstractLink*>& path, Vertex* pSrc, UINT connType);
	...
	bool noFullLinks(list<AbstractLink*>&pPath);
	...
	bool BBU_ProvisionHelper_Unprotected_BIG(Connection * pCon, BandwidthGranularity bwd);

	void printSecondBBUs();

	void resetGrooming();

	bool isLatencyExceeded();
...
private:
    ...
    UINT            m_nTimePolicy;			// 0 No Time - 1 CI - 2 PI --M	//-B: what do they mean???
	...
	MappedLinkList <UINT, Connection*> groomingConnections;

public:

		LightpathDB		m_hLightpathDB;	        // Lightpath info
		ConnectionDB	m_hConnectionDB;        // Connection info
